          ...
          {cells.map((date, i) => {
            ...
            return (
              ...
              >
                ...
                {showRangeLeft && (
                  <div className="absolute left-0 top-1 bottom-1 w-1/2 bg-violet-100" />
                )}
                {/* Range background: right half */}
                {showRangeRight && (
                  <div className="absolute right-0 top-1 bottom-1 w-1/2 bg-violet-100" />
                )}
                ...
                <span
                  className={`
                    ...
                    ${isStart || isEnd
                      ...
                      : isToday
                      ? "border-2 border-violet-500 text-violet-700 font-semibold"
                      : isHover && !isPast
  ...
  const panel = (
    ...
    >
      ...
      <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-6">
        <div className="text-sm text-slate-500">
          {depart && ret ? (
            <span>
              ...
              {nights && nights > 0 && (
                <span className="ml-2 text-violet-600 font-semibold">{nights} nacht{nights !== 1 ? "en" : ""}</span>
              )}
            </span>
          ) : depart ? (
            <span>
        ...
        <button
          onClick={onClose}
          className="px-5 py-2 bg-violet-600 text-white text-sm font-semibold rounded-xl hover:bg-violet-700 transition-colors shrink-0"
        >
          Toepassen
        </button>
      </div>
